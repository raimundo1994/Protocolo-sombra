...
from protocolo_sombra.ui.terminal import C
...
ALIASES = {
    ...
    "procurar": "revistar",
    # [MELHORIA 14] Ações em inglês
    "grab": "pegar", "pick": "pegar", "drop": "soltar",
    "examine": "examinar", "go": "mover", "walk": "mover",
    "inventory": "inventario", "merge": "combinar",
    # [MELHORIA 4] Comando voltar
    "voltar": "voltar", "back": "voltar", "retornar": "voltar",
...
def distorcer_comando(comando, sanidade):
    ...
    # [CORREÇÃO B7] Mapa bidirecional de direções
    def _trocar_direcao(c):
        mapa_dir = {"norte": "sul", "sul": "norte", "leste": "oeste", "oeste": "leste"}
        for d_orig, d_troca in mapa_dir.items():
            if d_orig in c:
                return c.replace(d_orig, d_troca)
        return c

    distorcoes = [
        # Troca direção (bidirecional)
        lambda c: _trocar_direcao(c) if any(d in c for d in ["norte", "sul", "leste", "oeste"]) else c,
        # Inverte ação
