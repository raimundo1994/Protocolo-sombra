...
from protocolo_sombra_v3.ui.hud import (
    verificar_conquistas, mostrar_conquistas,
    mostrar_diario, adicionar_nota, mostrar_notas
)
from protocolo_sombra_v3.entities.jogador import (
...
from protocolo_sombra_v3.engine.parser import (
    parsear, encontrar_item_fuzzy, encontrar_interacao_fuzzy
)
from protocolo_sombra_v3.engine.save_system import (
    salvar_jogo, carregar_jogo, carregar_meta,
    obter_ecos_run_anterior
)
from protocolo_sombra_v3.world.salas import criar_salas
from protocolo_sombra_v3.world.interacoes import INTERACOES_DATA, COMBINACOES
from protocolo_sombra_v3.narrative.eventos import (
...
from protocolo_sombra_v3.narrative.finais import (
  ...
  ╚═══════════════════════════════════════════════════════════════════════════════════╝
    ...
    def processar_comando(self, entrada):
        ...
        if verbo in ("norte", "sul", "leste", "oeste"):
            ...
            self.examinar(args) if args else self.descrever_sala()
        elif verbo == "pegar":
            self.pegar(args)
    ...
    def mover(self, direcao):
        ...
        if destino.trancada:
            ...
            if pode_abrir:
                ...
                if destino.requer_flag and not self.jogador.flags.get(destino.requer_flag):
                    print(f"{C.DIM}  Parece que um protocolo precisa ser ativado primeiro.{C.RESET}")
    ...
    def combinar_itens(self, args):
        ...
        combo_id = f"{item1_id}+{item2_id}"

        if combo_id in self.jogador.combinacoes_feitas or f"{item2_id}+{item1_id}" in self.jogador.combinacoes_feitas:
